...
<?php if($page_sub=='campaigns'){
		echo('<h1>'. $pg_title .' 
		<span class="help-block text-primary">'. newsletter_campaigns .'</span></h1><hr>'.
			  $pg_nav_buts.
			  $errText
			 );
if(DEMO_MODE){
...
$rndQry = '';
$opCamp = $myconn->query("SELECT ID FROM ". db_table_pref ."campaigns WHERE OID=". set_org_id ." AND campaign_type=0 ". $rndQry ."") or die(mysqli_error($myconn));
				...
				<tr>
					...
					<td>
						...
						if(date("Y-m-d H:i:s",strtotime($opRecsRs['launch_date']))>date("Y-m-d H:i:s")){
							echo('<br><span class="help-block txxs" data-countdown="'. setMyDate($opRecsRs['launch_date'],6) .'"></span>');
						}
						?>
					</td>
...
<?php }else if($page_sub=='add'){
		echo('<h1>'. $pg_title .' 
		<span class="help-block text-primary">'. newsletter_add_campaign .'</span></h1><hr>'.
			  $pg_nav_buts.
			  $errText
			 );
		echo('<div class="row">
...
<?php }else if($page_sub=='edit'){
		echo('<h1>'. $pg_title .' 
		<span class="help-block text-primary">'. newsletter_edit_campaign .'</span></h1><hr>'.
			  $pg_nav_buts.
			  $errText
			 );
